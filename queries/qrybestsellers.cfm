...
<cfif request.dbtype IS 'mysql'>
    <cfquery name = "qryBestSellers" datasource="#request.dsn#">
    SELECT * FROM products JOIN product_categories ON products.itemid = product_categories.itemid
    WHERE products.UnitsSold > <cfqueryparam value="0" cfsqltype="cf_sql_integer"> 
...
<cfif request.dbtype IS 'msaccess'>
    ...
    WHERE UnitsSold > <cfqueryparam value="0" cfsqltype="cf_sql_integer"> 
